        ...
        if (direction <= HexDirection.E && nextNeighbor != null) {
            ...
            if (cell.Elevation <= neighbour.Elevation) {
                if (cell.Elevation <= nextNeighbor.Elevation) {
                    TriangulateCorner(v2, cell, v4, neighbour, v5, nextNeighbor);
                } else {
                    TriangulateCorner(v5, nextNeighbor, v2, cell, v4, neighbour);
                }
            } else if (neighbour.Elevation <= nextNeighbor.Elevation) {
                TriangulateCorner(v4, neighbour, v5, nextNeighbor, v2, cell);
            } else {
                TriangulateCorner(v5, nextNeighbor, v2, cell, v4, neighbour);
            }
            //AddTriangle(v2, v4, v5);//v2 + HexMetrics.GetBridge(direction.Next()));
            //AddTriangleColor(cell.color, neighbour.color, nextNeighbor.color);
        }
    }
